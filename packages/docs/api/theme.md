...
| `nodeHighlightRecolours` | `true`                                                 | The same choice for a NODE: repaint it in `highlightFill`, or keep its own fill and lift it. Matters most on a sunburst, where a sector's colour IS which branch it belongs to — flooding it with one accent answers "which one is the pointer on" by deleting the answer to "what is it".           |
| `highlightLift`          | `0.1`                                                  | How far `nodeHighlightRecolours: false` shifts a lit node's fill, in OKLab lightness — positive lighter, negative darker. Perceptual, so one number reads as the same step on a pale leaf and a saturated root.                                                                                      |
