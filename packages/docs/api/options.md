...
| `weight`     | `(item) => number`          | —                         | What a node is worth, for the layouts that divide a fixed extent between siblings — the sunburst today. Read on the **leaves**; a parent's share is the sum of what is under it. Zero, negative and non-finite all count as zero — there is no per-leaf default of `1`, which would be an arbitrary quantity in somebody else's units. A tree where nothing is measured falls back to counting leaves. Turns a wheel of a file tree into a picture of where the disk went. |
